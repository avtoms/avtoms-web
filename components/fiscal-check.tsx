...
// Styles for the paper and its children. Global class names (not modules) so a second instance
// just re-declares them harmlessly. The page frame (.inv-root/.inv-bar) lives with the caller.
export const CHECK_CSS = `
  .inv-paper { background: #fff; border-radius: 12px; padding: 40px; color: #18181b; font-family: var(--ff-golos), system-ui, sans-serif; }
  ...
  .inv-tot { margin-top: 14px; margin-left: auto; width: 300px; max-width: 100%; font-size: 14px; }
  ...
  .inv-save { margin-top: 12px; margin-left: auto; width: 300px; max-width: 100%; background: #dcfce7; color: #166534; border-radius: 9px; padding: 9px 12px; font-size: 13.5px; font-weight: 700; display: flex; justify-content: space-between; }
  ...
  .inv-foot { display: flex; justify-content: space-between; align-items: flex-end; gap: 24px; margin-top: 28px; padding-top: 18px; border-top: 1px solid #e4e4e7; flex-wrap: wrap; }
...
export function FiscalCheck({ invoice, wo, shop, innerRef }: {
  ...
  // Per line: the list (menu) price is the discount reference; unit_price is what the customer
  // actually pays. A custom line (no menu origin) simply has no discount.
  const rows = (wo?.lineItems ?? []).map((it) => {
    const qty = it.quantity || 0;
    const actualUnit = num(it.unitPrice);
    const listUnit = num(it.defaultPrice) > actualUnit ? num(it.defaultPrice) : actualUnit;
    return { it, qty, actualUnit, listUnit, disc: Math.max(0, (listUnit - actualUnit) * qty), lineSum: actualUnit * qty };
  });
  ...
  return (
    <>
      ...
      <div className="inv-paper" ref={innerRef}>
        <div className="inv-h">
          <div>
        ...
        {/* Owner-only breakdown: cost + income (doxod). Screen-only — never on the printed check. */}
