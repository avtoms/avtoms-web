...
export function CreateWOModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [vf, setVf] = useState({ plate: "", make: "", model: "", year: "", vin: "", mileage: "" });

  React.useEffect(() => {
    if (open) { setMode("search"); setQ(""); setMatches([]); setCf({ name: "", phone: "", telegram: "", language: "uz" }); setVf({ plate: "", make: "", model: "", year: "", vin: "", mileage: "" }); }
  }, [open]);
    ...
    <Modal open={open} onClose={onClose} title={t("new_wo")} maxWidth={480}
      ...
      {mode === "search" ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 340, overflowY: "auto" }}>
            ...
            {matches.map((v) => (
              <button key={v.id} disabled={busy} onClick={() => createFor(v.id)} className="an-row-btn" style={{ display: "flex", alignItems: "center", gap: 13, padding: "12px 14px", border: "1px solid var(--line)", borderRadius: "var(--radius-sm)", background: "var(--surface)", cursor: "pointer", fontFamily: "var(--font-sans)", textAlign: "left" }}>
                ...
                <span style={{ fontFamily: "var(--font-mono)", fontWeight: 700, color: "var(--ink-2)", fontSize: 14, background: "var(--surface-2)", padding: "5px 9px", borderRadius: 7 }}>{v.plate}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <PlateField value={vf.plate} onChange={(p) => setVf((s) => ({ ...s, plate: p }))} label={t("plate")} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
